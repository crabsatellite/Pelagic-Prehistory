...
public final class PPEvents {
    ...
    public static void register(IEventBus modEventBus) {
        // ModHandler is now handled differently in NeoForge - events are registered on event bus
        NeoForge.EVENT_BUS.register(ForgeHandler.class);
    }

    public static final class ForgeHandler {

        private static final TagKey<EntityType<?>> ENTITY_TYPE_PREVENTS_DROWNED = TagKey.create(
                Registries.ENTITY_TYPE, ResourceLocation.fromNamespaceAndPath(PelagicPrehistory.MODID, "prevents_drowned"));
        ...
        public static void onLivingCheckSpawn(final FinalizeSpawnEvent event) {
            ...
            if (event.getEntity().getType() == EntityType.DROWNED
                ...
                level.getEntities().get(EntityTypeTagTest.forTag(ENTITY_TYPE_PREVENTS_DROWNED), aabb, e -> {
